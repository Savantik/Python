"""
In this task you are fish in a pond that needs to survive by eating other fish. You can only eat fish that are the same size or smaller than yourself. 
...
shoal = str(input('Give a chain of fish with levels 1-9: '))
def fish(shoal):
    xp, size, cap = 0, 1, 4
    for elem in sorted(map(int, shoal)):
        if elem > size: 
            break
        xp += elem
        if xp >= cap:
